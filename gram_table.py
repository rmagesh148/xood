...
def load_gram():
    ...
    df = pd.read_csv("benchmarks/table.csv", header=[0, 1])
    df.cols = mux
    return df
...
def generate(table, indist_model="cifar100_resnet", method="lr"):
    ...
    path = f"results/{indist_model}/summary_x-ood-{method}"
...
def generate_table(name):
    ...
    if exists("benchmarks/fulltable.pkl"):
        ...
        fulltable = table
    fulltable.to_pickle("benchmarks/fulltable.pkl")


def main():
    try:
        os.remove("benchmarks/fulltable.pkl")
...
def generate_latex(model_name):
    df = pd.read_pickle("benchmarks/fulltable.pkl")
    ...
    arrays = [df[("in_dist", "name")], df[("out_dist", "name")]]
    index = pd.MultiIndex.from_arrays(arrays, names=('In Dist', 'Out Dist'))
    data = {
        'TNR (95% TPR)': res[1],
        'AUROC': res[0],
        'Detection Acc.': res[2]
    ...
    df.to_csv(f"benchmarks/{model_name}.csv")
    with open(f"benchmarks/{model_name}.tex", "w") as fo:
